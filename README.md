...
> whole setup. The committed file ships with Andamio's own live rule (one
> `credential` rule gating the Andamio Issuer channel) as a **working example**;
> swap in your own course id, credential slt hash, and Discord role id and you
> are done. `config/role-mappings.example.json` is a separate, annotated
> reference showing all three rule types — copy from it if you want more rules.
...
### The rule shape (what ships)

The committed `role-mappings.json` holds Andamio's live rule as a working
example. To gate your own channel, replace the values with yours:

```json
[
  {
    "type": "credential",
    "course_id": "<your Andamio course id>",
    "slt_hash": "<the credential's slt hash>",
    "role_id": "<your Discord role id>",
    "label": "Your Credential Name",
    "earn_url": "https://app.andamio.io/course/<your course id>/<module>/assignment"
  }
]
```

Set the course id, credential slt hash, and your Discord role id, and the bot
gates that channel on that credential.
