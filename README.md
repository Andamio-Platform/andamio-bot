...
| `/progress` | Shows the connected member their per-module progress in an enrolled course, with status glyphs (✅ accepted · 🎓 credential claimed · 📝 in progress · ⬜ not started · ❌ refused). `course:<…>` **autocompletes** the member's enrolled, surfaced courses; the optional `view:opportunities` filters to just the open assignments (⬜/❌). Requires a connection (shows the **Connect** button when missing/expired); ephemeral, and display-only — never affects role gating. |
