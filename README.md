...
| `/preview` | Previews a course's modules and a chosen module's lesson or assignment as an ephemeral embed. `course:<…>` selects a course (curated by `COURSE_DISPLAY_NAMES`, like `/credentials`); the optional `module:<…>` **autocompletes** the course's on-chain modules. With no module it lists the on-chain modules; with one it renders the lesson (or the assignment when the module has none) as a title plus a plain-text excerpt. Public content — renders identically before connecting and degrades gracefully when Andamio is down. |
| `/progress` | The connected member's Andamio progress, at two levels. With no `course`, an overview of the credentials they have earned across completed courses (including ones they are no longer enrolled in) plus their courses in progress. With a `course`, that course's per-module status glyphs (✅ accepted · 🎓 credential claimed · 📝 in progress · ⬜ not started · ❌ refused), and the optional `view:opportunities` narrows to just the open assignments (⬜/❌). `course:<…>` **autocompletes** the member's enrolled **and completed** surfaced courses, so a finished course can be drilled into too. Requires a connection (shows the **Connect** button when missing or expired). Ephemeral and display-only: never affects role gating. |
