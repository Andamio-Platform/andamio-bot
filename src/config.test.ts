...
function validEnv(): NodeJS.ProcessEnv {
  return {
    ...
    GUILD_ID: '987654321',
    SCAN_BASE_URL: 'https://preprod.api.andamio.io',
...
describe('loadConfig', () => {
  it('parses a valid env into a typed config', () => {
    ...
    expect(config.scanBaseUrl).toBe('https://preprod.api.andamio.io');
  ...
  it('strips a trailing slash from base URLs', () => {
    const env = validEnv();
    env.SCAN_BASE_URL = 'https://preprod.api.andamio.io/';
  ...
  it('rejects a malformed SCAN_BASE_URL', () => {
    const env = validEnv();
    env.SCAN_BASE_URL = 'not a url';
    expect(() => loadConfig(env)).toThrow(/SCAN_BASE_URL/);
  });
